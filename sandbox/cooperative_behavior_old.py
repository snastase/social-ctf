...
follow_ts = following_teammate(wrap_f, map_id=map_id, matchup_id=matchup_id,
                               repeat_id=repeat_id, player_id=player_id)

escort_ts = escort(wrap_f, map_id=map_id, matchup_id=matchup_id,
                    repeat_id=repeat_id, player_id=player_id)

cycle_ts = cycling(wrap_f, map_id=map_id, matchup_id=matchup_id,
                     repeat_id=repeat_id, player_id=player_id)


# Compute whether escorting teammate with flag
#assists = assist(wrap_f, map_id=map_id, matchup_id=matchup_id,
#                 repeat_id=repeat_id, player_id=player_id,
#                 min_behavior_length=15, min_approach=1.0).squeeze()

# Compute whether escorting teammate with flag
#forts = hold_fort(wrap_f, map_id=map_id, matchup_id=matchup_id,
#                  repeat_id=repeat_id, player_id=player_id,
#                  min_behavior_length=15, base_radius=3).squeeze()

# Compute whether teammates are sieging opponent base
#sieges = siege(wrap_f, map_id=map_id, matchup_id=matchup_id,
#               repeat_id=repeat_id, player_id=player_id,
#               min_behavior_length=15, base_radius=3).squeeze()

# Compute whether teammates are sieging opponent base
#redoubts = redoubt(wrap_f, map_id=map_id, matchup_id=matchup_id,
#                   repeat_id=repeat_id, player_id=player_id,
#                   min_behavior_length=15, base_radius=3).squeeze()
...
#basecamps = camp_own_base(wrap_f, map_id=0, matchup_id=0,
#                          repeat_id=slice(None), player_id=player_id,
#                          min_behavior_length=15,
...
feature_labels = ['health', 'position', 'value',
...
features['distance from teammate'] = features.pop(
    'player_from_teammate_xy_distance')
features['distance from opponent base'] = features.pop(
    'player_from_opponent_base_xy_distance')[0]
...
for i, (ax, behavior) in enumerate(zip(axs[len(features) + 1:],
                                       behaviors)):
    ax.plot(behaviors[behavior][player_id], c='.5')
    ax.yaxis.set_ticks([])
    ax.set_xlim(0, 4500)
    ax.set_xlabel('time')
    ...
    #    ax.xaxis.set_ticks([])
sns.despine()
plt.savefig(f'figures/behavior-stack_ts.png',
...
for team, ax in zip(teams, axs[:2]):
    ax.plot(lstms_pca[team[0], :, pc_id],
            c=player_colors[team[0]], alpha=.7, lw=.8)
    ax.plot(lstms_pca[team[1], :, pc_id],
...
for team, ax in zip(teams, axs[2:]):
    ax.plot(behaviors[behavior][team[0]],
...
iscfs = np.load(f'results/iscfs_matchup-{matchup_id}_'
                f'map-{map_id}_repeat-{repeat_id}.npy')[..., pc_id, pc_id]
teams = [[0, 1], [2, 3]]
coop_ids = [0, 5]
...
fig, axs = plt.subplots(6, 1, figsize=(12, 5.5), sharex=True,
                        gridspec_kw={'height_ratios':
                                     [2.5, 2.5, 1.25, 2.5, 2.5, 1.25]})
for team, ax in zip(teams, axs[[0, 3]]):
    ax.plot(lstms_pca[team[0], :, pc_id],
            c=player_colors[team[0]], alpha=.7, lw=.8)
    ax.plot(lstms_pca[team[1], :, pc_id],
            c=player_colors[team[1]], alpha=.7, lw=.8)
    ...
    #ax.annotate(f'ISC: {lstm_pc_isc[0]:.3f}', (.995, .83),
    #            ha='right', xycoords='axes fraction')
for coop_id, team, ax in zip(coop_ids, teams, axs[[1, 4]]):
    ...
    #             rotation=0, ha='right', va='center')
    ax.tick_params(axis='x', length = 0)
for team, ax in zip(teams, axs[[2, 5]]):
    ax.plot(behaviors[behavior][team[0]],
            c=player_colors[team[0]])
    ax.yaxis.set_ticks([])
    ax.set_xlim(0, 4500)
...
ax.set_xlabel('time')
sns.despine()
plt.savefig(f'figures/isc-stack_ts.png', transparent=True,
            dpi=300, bbox_inches='tight')
...
from ctf_dataset.behaviors.heuristic import near_teammate
from ctf_dataset.behaviors.heuristic import camp_own_base, camp_opponent_base, running_forwards, running_backwards, approaching_own_base, approaching_opponent_base, approaching_own_flag, approaching_opponent_flag, approaching_teammate
...
following_luke = near_teammate(wrap_f, map_id=0, matchup_id=0, repeat_id=0,
                         min_behavior_length=30, teammate_radius=2)
...
camp_own = camp_own_base(wrap_f, map_id=0, matchup_id=0, repeat_id=0,
                         min_behavior_length=15, base_radius=1)
